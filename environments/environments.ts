...
function buildEnvVars(envDir: string, serverPort: number, expoPort: number): Record<string, string> {
    ...
    return {
        // Server
        NASTECH_MASTER_SECRET: "nastech-dev-secret",
        ...
        EXPO_PUBLIC_NASTECH_SERVER_URL: `http://localhost:${serverPort}`,
...
function buildEnvSh(name: string, envDir: string, serverPort: number, expoPort: number): string {
    ...
    lines.push("# Server");
    lines.push(`export NASTECH_MASTER_SECRET="${vars.NASTECH_MASTER_SECRET}"`);
    ...
    lines.push(`export EXPO_PUBLIC_NASTECH_SERVER_URL="${vars.EXPO_PUBLIC_NASTECH_SERVER_URL}"`);
...
async function main(): Promise<void> {
    ...
    switch (subcommand) {
        ...
        case "tailscale":
            ...
            break;
        default:
            console.log(`NasTech Environment Manager
