/**
 * Global configuration for nastech CLI
 * 
...
class Configuration {
  ...
  constructor() {
    ...
    // Directory configuration - Priority: NASTECH_HOME_DIR env > default home dir
    ...
    // webappUrl must follow the same chain as serverUrl, otherwise `nastech server`
    // self-host points the API at localhost but auth still opens the prod webapp.
    this.serverUrl =
      process.env.NASTECH_SERVER_URL ||
      readSettingsStringSync(this.settingsFile, 'serverUrl') ||
      'https://api.nastech.workers.dev'
    this.webappUrl =
      process.env.NASTECH_WEBAPP_URL ||
      readSettingsStringSync(this.settingsFile, 'webappUrl') ||
      'https://ba.nastech.workers.dev'

    this.isExperimentalEnabled = ['true', '1', 'yes'].includes(process.env.NASTECH_EXPERIMENTAL?.toLowerCase() || '');
    this.disableCaffeinate = ['true', '1', 'yes'].includes(process.env.NASTECH_DISABLE_CAFFEINATE?.toLowerCase() || '');
