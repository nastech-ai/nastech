...
const SERVER_PACKAGE_NAME = 'nastech-server-self-host';
const SETTINGS_WRITE_CONFIRM_FLAG = '--i-understand-this-will-modify-default-nastech-settings';
...
export async function handleServerCommand(args: string[]): Promise<void> {
    ...
    if (!artifacts) {
        console.error(chalk.red('Could not locate nastech-server.'));
        ...
        console.error(chalk.gray('    - sibling packages/nastech-server/sources/standalone.ts in the monorepo'));
    ...
    const env: NodeJS.ProcessEnv = {
        ...
        NASTECH_MASTER_SECRET: masterSecret,
        PORT: String(opts.port),
        HOST: opts.host,
    };
    if (staticDir) env.NASTECH_STATIC_DIR = staticDir;
    env.NASTECH_INJECT_HTML_CONFIG = JSON.stringify({
        serverUrl,
    ...
    if (artifacts.bundled) {
        ...
        if (!prismaEngine) {
            ...
            if (artifacts.source === 'package') {
                ...
                console.error(chalk.gray(`  Try reinstalling ${SERVER_PACKAGE_NAME}, then run \`nastech server\` again.`));
            } else {
                console.error(chalk.gray('  Expected @prisma/engines to be available near the nastech package.'));
                console.error(chalk.gray('  Try reinstalling nastech, then run `nastech server` again.'));
            }
            process.exit(1);
        }
    ...
    if (opts.persistServerUrl) {
        ...
        // Without this the CLI's auth flow would open the prod webapp (ba.nastech.workers.dev).
    ...
    console.log();
    console.log(chalk.green.bold(`✓ nastech-server starting at ${serverUrl}`));
    if (staticDir) {
        console.log(chalk.green(`  Open ${serverUrl} in your browser.`));
    }
    if (opts.persistServerUrl) {
        console.log(chalk.gray('  nastech CLI + daemon will use this server automatically (settings.serverUrl).'));
    }
    ...
    const exitCode = await new Promise<number>(resolve => {
        child.on('exit', code => {
            console.log(chalk.gray(`\nnastech-server exited (code ${code ?? 0})`));
            resolve(code ?? 0);
        });
    });
...
function showHelp() {
    console.log(`
${chalk.bold('nastech server')} - Run NasTech sync server + web app locally (self-host)

${chalk.bold('Usage:')}
  nastech server [--port 3005] [--host 127.0.0.1] [--reset] [--no-persist]
...
${chalk.bold('Notes:')}
  - Stores data in ${chalk.cyan('$NASTECH_HOME_DIR/server-data/')}
  ...
  - Use ${chalk.cyan('--no-persist')} to run without modifying default NasTech settings
...
async function ensureSettingsWriteAllowed(opts: ServerOptions, serverUrl: string): Promise<void> {
    ...
    const message =
        `nastech server will write settings.serverUrl and settings.webappUrl to ${serverUrl} ` +
        `in ${configuration.settingsFile}.`;

    if (!process.stdin.isTTY || !process.stderr.isTTY) {
        console.error(chalk.red('Refusing to modify default NasTech settings from a non-interactive run.'));
 ...
 *
 * pkgroll bundles into dist/, so __dirname at runtime is .../nastech-cli/dist; tools/ lives
 * at .../nastech-cli/tools. In rare layouts (e.g. running un-built source via tsx from src/
 * commands/), tools/ sits at .../nastech-cli/tools and __dirname is .../nastech-cli/src/commands,
...
function bundledBinaryName(): string {
    return process.platform === 'win32' ? 'nastech-server.exe' : 'nastech-server';
}
...
/**
 * Resolves the artifacts needed to spawn nastech-server.
 *
 * Order:
 *   1. nastech-server-self-host package (npm-installed local server artifact)
 *   2. Legacy bundled binary at tools/server/<platform>/nastech-server
 *   3. Source-mode fallback for monorepo dev: ../nastech-server/sources/standalone.ts via tsx
 */
...
function findSourceStandalone(): string | undefined {
    const candidates = [
        path.resolve(__dirname, '../../../nastech-server/sources/standalone.ts'),
        path.resolve(__dirname, '../../nastech-server/sources/standalone.ts'),
        path.resolve(process.cwd(), 'packages/nastech-server/sources/standalone.ts'),
        path.resolve(process.cwd(), '../nastech-server/sources/standalone.ts'),
    ];
...
function findWebappDir(): string | undefined {
    ...
    const candidates = [
        path.resolve(__dirname, '../../../nastech-app/dist'),
        path.resolve(__dirname, '../../nastech-app/dist'),
        path.resolve(process.cwd(), 'packages/nastech-app/dist'),
    ];
    for (const c of candidates) {
...
async function spawnAndWait(art: ServerArtifacts, env: NodeJS.ProcessEnv, args: string[]): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        child.on('exit', code => {
            if (code === 0) resolve();
            else reject(new Error(`nastech-server ${args[0]} exited with code ${code}`));
        });
    });
}
