...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('nastech auth')} - Authentication management

${chalk.bold('Usage:')}
  nastech auth login [--force]    Authenticate with NasTech
  nastech auth logout             Remove authentication and machine data
  nastech auth status             Show authentication status
  nastech auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'nastech auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  console.log(chalk.blue('This will log you out of NasTech'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use NasTech again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      // Remove entire nastech directory (as current logout does)
      if (existsSync(nasTechDir)) {
        rmSync(nasTechDir, { recursive: true, force: true });
      }

      console.log(chalk.green('✓ Successfully logged out'));
      console.log(chalk.gray('  Run "nastech auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "nastech auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "nastech auth login --force" to fix this'));
  }
