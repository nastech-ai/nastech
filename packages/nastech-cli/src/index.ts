...
/**
 * CLI entry point for nastech command
 * 
...
import { checkIfDaemonRunningAndCleanupStaleState, isDaemonRunningCurrentlyInstalledNasTechVersion, stopDaemon } from './daemon/controlClient'
import { getLatestDaemonLog } from './ui/logger'
import { killRunawayNasTechProcesses } from './daemon/doctor'
...
(async () => {
  ...
  if (!args.includes('--version')) {
    logger.debug('Starting nastech CLI with args: ', process.argv)
  }
  ...
  if (subcommand === 'doctor') {
    ...
    if (args[1] === 'clean') {
      if (args.slice(2).some(a => a === '--help' || a === '-h')) {
        console.log(`
${chalk.bold('nastech doctor clean')} - Kill all nastech-related processes (daemon + sessions)

${chalk.bold('Usage:')}
  nastech doctor clean
...
`)
      ...
      const result = await killRunawayNasTechProcesses()
    ...
    // Handle "nastech gemini model set <model>" command
    ...
    // Handle "nastech gemini model get" command
    ...
    // Handle "nastech gemini project set <project-id>" command
    if (geminiSubcommand === 'project' && args[2] === 'set' && args[3]) {
      ...
      try {
        ...
        // Try to get current user email from NasTech cloud token
    ...
    // Handle "nastech gemini project get" command
    if (geminiSubcommand === 'project' && args[2] === 'get') {
      try {
        ...
        if (config.googleCloudProject) {
          ...
          console.log('  nastech gemini project set <your-project-id>');
          console.log('');
    ...
    // Handle "nastech gemini project" (no subcommand) - show help
    if (geminiSubcommand === 'project' && !args[2]) {
      console.log('Usage: nastech gemini project <command>');
      console.log('');
    ...
    console.log(chalk.yellow('Note: "nastech logout" is deprecated. Use "nastech auth logout" instead.\n'));
    try {
    ...
    if (daemonSubcommand === 'list') {
      ...
      const child = spawnNasTechCLI(['daemon', 'start-sync'], {
        detached: true,
        stdio: 'ignore',
        env: process.env
      });
      ...
      console.log(`
${chalk.bold('nastech daemon')} - Daemon management

${chalk.bold('Usage:')}
  nastech daemon start              Start the daemon (detached)
  nastech daemon stop               Stop the daemon (sessions stay alive)
  nastech daemon status             Show daemon status
  nastech daemon list               List active sessions

  If you want to kill all nastech related processes run 
  ${chalk.cyan('nastech doctor clean')}

${chalk.bold('Note:')} The daemon runs in the background and manages Claude sessions.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('nastech doctor clean')}
`)
    }
    return;
  } else {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-h' || arg === '--help') {
        ...
        unknownArgs.push(arg)
      } else if (arg === '--nastech-starting-mode') {
        ...
        // NasTech-specific flag to disable chrome even if default is on
      } else if (arg === '--settings') {
        // Intercept --settings flag - NasTech uses this internally for session hooks
        const settingsValue = args[++i] // consume the value
        console.warn(chalk.yellow(`⚠️  Warning: --settings is used internally by NasTech for session tracking.`))
    ...
    if (showHelp) {
      console.log(`
${chalk.bold('nastech')} - Claude Code On the Go

${chalk.bold('Usage:')}
  nastech [options]         Start Claude with mobile control
  nastech auth              Manage authentication
  nastech resume            Resume a previous NasTech session by NasTech session ID
  nastech codex             Start Codex mode
  nastech gemini            Start Gemini mode (ACP)
  nastech acp               Start a generic ACP-compatible agent
  nastech connect           Connect AI vendor API keys
  nastech sandbox           Configure and manage OS-level sandboxing
  nastech notify            Send push notification
  nastech daemon            Manage background service that allows
                            to spawn new sessions away from your computer
  nastech doctor            System diagnostics & troubleshooting

${chalk.bold('Examples:')}
  nastech                    Start session
  nastech resume cmmij8      Resume a previous session by NasTech session ID
  nastech --yolo             Start with bypassing permissions
                            nastech sugar for --dangerously-skip-permissions
  nastech --chrome           Enable Chrome browser access for this session
  nastech --no-chrome        Disable Chrome even if default is on
  nastech --no-sandbox       Disable NasTech sandbox for this session
  nastech --js-runtime bun   Use bun instead of node to spawn Claude Code
  nastech --claude-env ANTHROPIC_BASE_URL=http://127.0.0.1:3456
                           Use a custom API endpoint (e.g., claude-code-router)
  nastech acp gemini         Start Gemini via generic ACP runner
  nastech acp -- opencode --acp
                           Start a custom ACP command
  nastech acp opencode --verbose
                           Print raw ACP backend/envelope events
  nastech auth login --force Authenticate
  nastech doctor             Run diagnostics

${chalk.bold('NasTech supports ALL Claude options!')}
  Use any claude flag with nastech as you would with claude. Our favorite:
...
`)
    ...
    // Show version
    if (showVersion) {
      console.log(`nastech version: ${packageJson.version}`)
...
async function handleNotifyCommand(args: string[]): Promise<void> {
  ...
  if (showHelp) {
    console.log(`
${chalk.bold('nastech notify')} - Send notification

${chalk.bold('Usage:')}
  nastech notify -p <message> [-t <title>]    Send notification with custom message and optional title
  nastech notify -h, --help                   Show this help

${chalk.bold('Options:')}
  -p <message>    Notification message (required)
  -t <title>      Notification title (optional, defaults to "NasTech")

${chalk.bold('Examples:')}
  nastech notify -p "Deployment complete!"
  nastech notify -p "System update complete" -t "Server Status"
  nastech notify -t "Alert" -p "Database connection restored"
`)
    return
  }
  ...
  if (!message) {
    ...
    console.log(chalk.gray('Run "nastech notify --help" for usage information.'))
    process.exit(1)
  }
  ...
  if (!credentials) {
    console.error(chalk.red('Error: Not authenticated. Please run "nastech auth login" first.'))
    process.exit(1)
  }
  ...
  try {
    ...
    // Use custom title or default to "NasTech"
    const notificationTitle = title || 'NasTech'
