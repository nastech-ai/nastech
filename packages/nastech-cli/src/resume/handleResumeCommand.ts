...
import { resolveNasTechSession, type ResumableNasTechSession } from './resolveNasTechSession';
...
export function parseResumeCommandArgs(args: string[]): { showHelp: boolean; sessionId: string } {
    ...
    if (args.length === 0) {
        throw new Error('NasTech session ID is required: nastech resume <session-id>');
    }
    if (args.length > 1) {
        throw new Error(`Unexpected arguments for nastech resume: ${args.slice(1).join(' ')}`);
    }
...
export function buildResumeLaunch(session: ResumableNasTechSession, options: ResumeLaunchOptions = {}): ResumeLaunch {
    const { metadata } = session;
    const flavor = resolveFlavor(metadata);

    if (flavor === 'codex') {
        if (!metadata.codexThreadId) {
            throw new Error(`NasTech session ${session.id} is missing its Codex thread ID.`);
        }
    ...
    if (flavor === 'claude') {
        if (!metadata.claudeSessionId) {
            throw new Error(`NasTech session ${session.id} is missing its Claude session ID.`);
        }
        const args = ['claude'];
        if (options.claudeStartingMode) {
            args.push('--nastech-starting-mode', options.claudeStartingMode);
        }
        if (options.startedBy) {
            args.push('--started-by', options.startedBy);
        }
    ...
    throw new Error(`NasTech session ${session.id} uses unsupported flavor "${metadata.flavor ?? 'unknown'}".`);
}

export function formatResumeHelp(): string {
    return [
        'nastech resume - Resume a previous NasTech session',
        '',
        'Usage:',
        '  nastech resume <nastech-session-id>',
        '',
        'Examples:',
        '  nastech resume cmmij8olq00dp5jcxr3wtbpau',
        '  nastech resume cmmij8',
        '',
...
function spawnResumeChild(launch: ResumeLaunch): Promise<number | null> {
    return new Promise((resolve, reject) => {
        const child = spawnNasTechCLI(launch.args, {
            cwd: launch.cwd,
            env: process.env,
            stdio: 'inherit',
        });
...
export async function handleResumeCommand(args: string[]): Promise<void> {
    ...
    const session = await resolveNasTechSession(parsed.sessionId);
