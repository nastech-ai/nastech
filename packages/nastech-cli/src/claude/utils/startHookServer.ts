 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.nastech/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `nastech` (fresh start) - new session created
 * - `nastech --continue` - continues last session (may fork)
 * - `nastech --resume` - interactive picker, then resume
 * - `nastech --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple NasTech processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between NasTech process and Claude session.
 */
