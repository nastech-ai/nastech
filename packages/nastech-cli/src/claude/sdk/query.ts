...
import { resolveNasTechEntrypoint } from './nasTechEntrypoint'
...
export function query(params: { prompt: QueryPrompt; options?: QueryOptions }): Query {
    ...
    // so sessions NasTech starts/continues remain visible to a plain
    // `claude --resume` picker. The agent SDK would otherwise default to
    // CLAUDE_CODE_ENTRYPOINT="sdk-ts" and the picker would hide every NasTech
    // session. See nastech-ai/nastech#1202.
    const env: Record<string, string> = {}
    for (const [key, value] of Object.entries(process.env)) {
        if (typeof value === 'string') env[key] = value
    }
    env.CLAUDE_CODE_ENTRYPOINT = resolveNasTechEntrypoint(process.env.CLAUDE_CODE_ENTRYPOINT)
