...
import { startNasTechServer } from '@/claude/utils/startNasTechServer';
...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    logger.debugLargeJson('[START] NasTech process started', getEnvironmentInfo());
    logger.debug(`[START] Options: startedBy=${options.startedBy}, startingMode=${options.startingMode}`);

    // Validate daemon spawn requirements - fail fast on invalid config
    if (options.startedBy === 'daemon' && options.startingMode === 'local') {
        throw new Error('Daemon-spawned sessions cannot use local/interactive mode. Use --nastech-starting-mode remote or spawn sessions directly from terminal.');
    }
    ...
    if (!machineId) {
        console.error(`[START] No machine ID found in settings, which is unexpected since authAndSetupMachineIfNeeded should have created it. Please report this issue on https://github.com/nastech-ai/nastech-cli/issues`);
        process.exit(1);
    }
    ...
    const forkedFromSessionId = process.env.NASTECH_FORKED_FROM_SESSION_ID;
    const forkedFromMessageId = process.env.NASTECH_FORKED_FROM_MESSAGE_ID;
    ...
    let metadata: Metadata = {
        ...
        nastechToolsDir: resolve(projectPath(), 'tools', 'unpacked'),
    ...
    const reconnectSessionId = process.env.NASTECH_RECONNECT_SESSION_ID;
    const reconnectKeyBase64 = process.env.NASTECH_RECONNECT_ENCRYPTION_KEY;
    const reconnectVariant = process.env.NASTECH_RECONNECT_ENCRYPTION_VARIANT as 'legacy' | 'dataKey' | undefined;
    const reconnectSeq = process.env.NASTECH_RECONNECT_SEQ;
    const reconnectMetadataVersion = process.env.NASTECH_RECONNECT_METADATA_VERSION;
    const reconnectAgentStateVersion = process.env.NASTECH_RECONNECT_AGENT_STATE_VERSION;
    ...
    // Fork backfill: when this NasTech session was just spawned as a fork
    // of another (NASTECH_FORK_CLAUDE_SESSION_ID is set by the daemon at
    ...
    // historical messages back to the NasTech client — so without an
    ...
    //
    // Skipped on reconnect (NASTECH_RECONNECT_*) — that path reattaches
    // to the existing NasTech session, where the server already has every
    // message it needs.
    const forkClaudeSessionId = process.env.NASTECH_FORK_CLAUDE_SESSION_ID;
    if (!reconnectSessionId && forkClaudeSessionId) {
        ...
        try {
            ...
            // Bind the new NasTech session to the forked Claude UUID up
    ...
    // alongside this NasTech session. App-sent prompts also land in the
    ...
    // to the running NasTech session. SDK-emitted assistant + tool_result
    ...
    // Start NasTech MCP server
    const nasTechServer = await startNasTechServer(session);
    logger.debug(`[START] NasTech MCP server started at ${nasTechServer.url}`);
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: nasTechServer.toolNames.map(toolName => `mcp__nastech__${toolName}`),
    ...
    // Stop NasTech MCP server
    nasTechServer.stop();
    logger.debug('Stopped NasTech MCP server');
