...
export async function runCodex(opts: {
    ...
    // Start NasTech MCP server (HTTP) and prepare STDIO bridge config for Codex
    ...
    const bridgeEntrypoint = join(projectPath(), 'bin', 'nastech-mcp.mjs');
    const mcpServers = {
        nastech: {
            command: process.execPath,
            args: ['--no-warnings', '--no-deprecation', bridgeEntrypoint, '--url', nasTechServer.url]
        }
    } as const;
    let first = true;
    ...
    try {
        ...
        // Stop NasTech MCP server
        logger.debug('[codex]: nasTechServer.stop');
        nasTechServer.stop();
