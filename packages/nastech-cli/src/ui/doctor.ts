 ...
 * for nastech CLI including configuration, daemon status, logs, and links
 */
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        ...
        NASTECH_PROJECT_ROOT: process.env.NASTECH_PROJECT_ROOT,
...
/**
 * Slim daemon status output for `nastech daemon status`
 */
export async function runDoctorDaemon(): Promise<void> {
    ...
    console.log(chalk.gray('\nRun `nastech doctor` for full diagnostics.\n'));
}
...
export async function runDoctorCommand(): Promise<void> {
    ...
    try {
        ...
        if (allProcesses.length > 0) {
            ...
            if (allProcesses.length > 1) {
                console.log(chalk.bold('\n💡 Process Management'));
                console.log(chalk.gray('To clean up runaway processes: nastech doctor clean'));
            }
        } else {
            console.log(chalk.red('❌ No nastech processes found'));
        }
    } catch (error) {
    ...
    const wrapperPath = join(projectRoot, 'bin', 'nastech.mjs');
    ...
    console.log(`NASTECH_HOME_DIR: ${env.NASTECH_HOME_DIR ? chalk.green(env.NASTECH_HOME_DIR) : chalk.gray('not set')}`);
    console.log(`NASTECH_SERVER_URL: ${env.NASTECH_SERVER_URL ? chalk.green(env.NASTECH_SERVER_URL) : chalk.gray('not set')}`);
    ...
    console.log(chalk.bold('\n🐛 Support & Bug Reports'));
    console.log(`Report issues: ${chalk.blue('https://github.com/nastech-ai/nastech-cli/issues')}`);
    console.log(`Documentation: ${chalk.blue('https://ba.nastech.workers.dev/')}`);

    // ── Concise useful info last (visible without scrolling) ──

    // Basic info
    console.log(chalk.bold('\n📋 Basic Information'));
    console.log(`NasTech CLI Version: ${chalk.green(packageJson.version)}`);
