...
describe('Api server error handling', () => {
    ...
    describe('getOrCreateSession', () => {
        it('should return null when NasTech server is unreachable (ECONNREFUSED)', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('⚠️  NasTech server unreachable')
            );

            consoleSpy.mockRestore();
        });

        it('should return null when NasTech server cannot be found (ENOTFOUND)', async () => {
        ...
        it('should return null when NasTech server times out (ETIMEDOUT)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
