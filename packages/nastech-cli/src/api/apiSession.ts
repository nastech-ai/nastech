...
import { type SessionEnvelope, type SessionTurnEndStatus } from '@nastech-ai/nastech-wire';
import {
...
export class ApiSessionClient extends EventEmitter {
     ...
     * Legacy sessions use deriveKey(masterSecret, 'NasTech Blobs', ['master']).
     * DataKey sessions use deriveKey(dataKey, 'NasTech Blobs', ['session']).
     */
    async getBlobKey(): Promise<Uint8Array> {
        if (!this.blobKey) {
            const path = this.encryptionVariant === 'dataKey' ? ['session'] : ['master'];
            this.blobKey = await deriveKey(this.encryptionKey, 'NasTech Blobs', path);
        }
        return this.blobKey;
    }
