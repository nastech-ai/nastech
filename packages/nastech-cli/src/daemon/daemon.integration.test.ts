 ...
 *
 * This file boots one authenticated NasTech environment and restarts the daemon
...
describe('Daemon Integration Tests', { timeout: 180_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnNasTechCLI(['daemon', 'start'], {
      stdio: 'ignore',
    });
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    // Spawn a real nastech process that looks like it was started from terminal
    const terminalNasTechProcess = spawnNasTechCLI([
    ...
    if (!terminalNasTechProcess || !terminalNasTechProcess.pid) {
      throw new Error('Failed to spawn terminal nastech process');
    }
    ...
    expect(terminalSession.startedBy).toBe('nastech directly - likely by user from terminal');
    ...
    try {
      terminalNasTechProcess.kill('SIGTERM');
    } catch (e) {
   ...
   * 6. Daemon spawns new daemon via spawnNasTechCLI(['daemon', 'start'])
   ...
   * 
   * This simulates what happens during `npm upgrade nastech`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/nastech/ with NEW version files
  ...
  it.skip('[skipped] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.NASTECH_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when nastech is uninstalled
