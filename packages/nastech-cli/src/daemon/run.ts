...
import { detectResumeSupport } from '@/resume/localNasTechAgentAuth';
...
// Suffix host with `-dev` for the NASTECH_VARIANT=dev variant so the dev daemon
// is visually distinct from the stable one in the machine list (they otherwise
// share the same hostname and look identical).
const hostSuffix = process.env.NASTECH_VARIANT === 'dev' ? '-dev' : '';
...
export async function startDaemon(): Promise<void> {
  ...
  let requestShutdown: (source: 'nastech-app' | 'nastech' | 'os-signal' | 'exception', errorMessage?: string) => void;
  let resolvesWhenShutdownRequested = new Promise<({ source: 'nastech-app' | 'nastech' | 'os-signal' | 'exception', errorMessage?: string })>((resolve) => {
  ...
  try {
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        if (options.parentSessionId) {
          extraEnv.NASTECH_FORKED_FROM_SESSION_ID = options.parentSessionId;
        }
        if (options.forkedFromMessageId) {
          extraEnv.NASTECH_FORKED_FROM_MESSAGE_ID = options.forkedFromMessageId;
        }
        // For fork: spawned NasTech CLI needs to know which Claude JSONL to
        // backfill into the fresh NasTech session row. Without this, the
        ...
        if (options.resumeClaudeSessionId) {
          extraEnv.NASTECH_FORK_CLAUDE_SESSION_ID = options.resumeClaudeSessionId;
        }
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          // nastech-pass-through `--resume <id>` argument the same way.
          const resumeFragment = options.resumeClaudeSessionId && agent === 'claude'
            ? ` --resume ${shellescape(options.resumeClaudeSessionId)}`
            : '';
          const fullCommand = `node --no-warnings --no-deprecation ${cliPath} ${agent} --nastech-starting-mode remote --started-by daemon${resumeFragment}`;
          ...
          const windowName = `nastech-${Date.now()}-${agent}`;
        ...
        if (!useTmux) {
          ...
          const args = [
            agentCommand,
            '--nastech-starting-mode', 'remote',
            '--started-by', 'daemon'
          ];

          // resumeClaudeSessionId attaches the new NasTech session to a pre-existing
          // Claude conversation file (used by the fork / duplicate flow). We pass
          // it through `--resume <id>` as NasTech's existing pass-through to claude.
    ...
    const spawnTrackedHappyProcess = ({
      ...
      const nasTechProcess = spawnNasTechCLI(args, {
        cwd,
        detached: true,
        stdio: 'ignore',
        env,
      ...
      if (!nasTechProcess.pid) {
        logger.debug('[DAEMON RUN] Failed to spawn process - no PID returned');
        return Promise.resolve({
          type: 'error',
          errorMessage: 'Failed to spawn NasTech process - no PID returned'
        });
      }

      logger.debug(`[DAEMON RUN] Spawned process with PID ${nasTechProcess.pid}`);

      const trackedSession: TrackedSession = {
        startedBy: 'daemon',
        pid: nasTechProcess.pid,
        childProcess: nasTechProcess,
        directoryCreated,
        message,
      };

      pidToTrackedSession.set(nasTechProcess.pid, trackedSession);

      nasTechProcess.on('exit', (code, signal) => {
        logger.debug(`[DAEMON RUN] Child PID ${nasTechProcess.pid} exited with code ${code}, signal ${signal}`);
        if (nasTechProcess.pid) {
          onChildExited(nasTechProcess.pid);
        }
      });

      nasTechProcess.on('error', (error) => {
      ...
      logger.debug(`[DAEMON RUN] Waiting for session webhook for PID ${nasTechProcess.pid}`);

      return new Promise((resolve) => {
        const timeout = setTimeout(() => {
          pidToAwaiter.delete(nasTechProcess.pid!);
          logger.debug(`[DAEMON RUN] Session webhook timeout for PID ${nasTechProcess.pid}`);
          resolve({
            type: 'error',
            errorMessage: `Session webhook timeout for PID ${nasTechProcess.pid}`
          });
        }, 15_000);
      ...
      try {
        ...
        return spawnTrackedHappyProcess({
          ...
          env: {
            ...
            NASTECH_RECONNECT_METADATA_VERSION: String(tracked.encryption.metadataVersion),
            NASTECH_RECONNECT_AGENT_STATE_VERSION: String(tracked.encryption.agentStateVersion),
          },
        });
      } catch (error) {
    ...
    // when npm replaces `dist/index.mjs` on disk (= the user ran `npm i -g nastech`).
    ...
    // diverged from the bundled version (e.g. `nastech-coder@0.13.1` deprecation
    ...
    apiMachine.setRPCHandlers({
      ...
      resumeSession,
      stopSession,
      requestShutdown: () => requestShutdown('nastech-app')
    });
    ...
    // 4. Write heartbeat
    const heartbeatIntervalMs = parseInt(process.env.NASTECH_DAEMON_HEARTBEAT_INTERVAL || '60000');
    ...
    // Setup signal handlers
    const cleanupAndShutdown = async (source: 'nastech-app' | 'nastech' | 'os-signal' | 'exception', errorMessage?: string) => {
