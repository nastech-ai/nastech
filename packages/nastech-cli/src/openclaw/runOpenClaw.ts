 ...
 * The daemon spawns this as: `node dist/index.mjs openclaw --nastech-starting-mode remote --started-by daemon`
 *
 * Connects to an OpenClaw gateway via WebSocket, translates the gateway protocol
 * to NasTech's AgentMessage format, and forwards everything through the session pipeline.
 */
...
export async function runOpenClaw(opts: RunOpenClawOptions): Promise<void> {
  ...
  if (response) {
    log(`NasTech Session ID: ${response.id}`);
  }
