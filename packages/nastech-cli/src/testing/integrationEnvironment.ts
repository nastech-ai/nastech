...
export function applyEnvironmentToProcess(env: IntegrationEnvironment) {
    process.env.NASTECH_SERVER_URL = `http://localhost:${env.serverPort}`;
    process.env.NASTECH_WEBAPP_URL = `http://localhost:${env.expoPort}`;
    process.env.NASTECH_HOME_DIR = join(env.envDir, 'cli', 'home');
    process.env.NASTECH_PROJECT_DIR = env.projectPath;
    process.env.NASTECH_VARIANT = 'dev';
    process.env.DEBUG = '1';
}
