...
export interface GeminiBackendOptions extends AgentFactoryOptions {
  ...
  /** OAuth token from NasTech cloud (via 'nastech connect gemini') - highest priority */
  cloudToken?: string;
...
export function createGeminiBackend(options: GeminiBackendOptions): GeminiBackendResult {
  ...
  // 1. NasTech cloud OAuth token (via 'nastech connect gemini') - highest priority
  ...
  let apiKey = options.cloudToken       // 1. NasTech cloud token (passed from runGemini)
  ...
  if (!apiKey) {
    logger.warn(`[Gemini] No API key found. Run 'nastech connect gemini' to authenticate via Google OAuth, or set ${GEMINI_API_KEY_ENV} environment variable.`);
  }
  ...
  const backendOptions: AcpBackendOptions = {
    ...
    hasChangeTitleInstruction: (prompt: string) => {
      ...
      return lower.includes('change_title') ||
             ...
             lower.includes('mcp__nastech__change_title');
    },
  };
