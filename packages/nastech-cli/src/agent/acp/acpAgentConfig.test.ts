...
describe('resolveAcpAgentConfig', () => {
  ...
  it('throws with helpful usage when no args are provided', () => {
    expect(() => resolveAcpAgentConfig([])).toThrow('Usage: nastech acp <agent-name> or nastech acp -- <command> [args]');
  });

  it('throws when separator form omits command', () => {
    expect(() => resolveAcpAgentConfig(['--'])).toThrow('Missing command after "--". Usage: nastech acp -- <command> [args]');
  });
});
