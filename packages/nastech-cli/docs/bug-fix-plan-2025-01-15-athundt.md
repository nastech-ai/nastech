# Minimal Fix Plan for NasTech-CLI Bugs with TDD
# Date: 2025-01-15
...
**Problem**: When running `./bin/nastech.mjs --continue`, Claude CLI returns error:
```
Error: --session-id cannot be used with --continue or --resume
```

**Root Cause Analysis**:
- This is a Claude Code 2.0.64+ design constraint, NOT a nastech-cli bug
- NasTech-CLI generates a NEW session ID and adds `--session-id <uuid>` for all local sessions
...
2. **Remote Mode (No conflict)**:
   ...
   → No --session-id added by nastech-cli
   → Works fine
   ```
...
## Bug 2: NasTech Server Unavailability Crash

**Problem**: NasTech-CLI crashes when NasTech API server is unreachable

**Server Details**:
- Default server: `https://api.nastech.workers.dev`
- Environment variable: `NASTECH_SERVER_URL` (overrides default)
- Local development: `http://localhost:3005`
- The server handles session management and real-time communication for NasTech CLI
...
try {
    ...
    console.log('⚠️  Cannot connect to NasTech server - continuing in local mode');
...
```typescript
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        console.log('⚠️  NasTech server unreachable - working in offline mode');
...
describe('ApiSessionClient connection handling', () => {
    it('should handle socket connection failure gracefully', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            '⚠️  Cannot connect to NasTech server - continuing in local mode'
        );
...
describe('Api server error handling', () => {
    it('should return null when NasTech server is unreachable', async () => {
        const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            '⚠️  NasTech server unreachable - working in offline mode'
        );
...
5. **Refactor Phase**:
   - Add session ID extraction for --continue (future enhancement):
     ...
     - Update NasTech's session metadata with Claude's ID
   - Ensure code is clean and minimal

6. **Manual Verification**:
   ```bash
   # Test Bug 1 fix:
   ./bin/nastech.mjs --continue  # Should work without error
   # Verify mobile/daemon still work with session ID

   # Test Bug 2 fix:
   NASTECH_SERVER_URL=http://invalid:9999 ./bin/nastech.mjs  # Should show warning, not crash
...
**Bug 1 Fixed**:
- Test: `./bin/nastech.mjs --continue` exits with code 0
- No "session-id cannot be used" error

**Bug 2 Fixed**:
- Test: `NASTECH_SERVER_URL=http://invalid:9999 ./bin/nastech.mjs` shows warning message
- Process continues in local mode instead of crashing
- Clear user feedback: "⚠️ NasTech server unreachable - working in offline mode"
