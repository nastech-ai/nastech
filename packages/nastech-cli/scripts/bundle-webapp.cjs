...
/**
 * Runs `expo export -p web` in packages/nastech-app and copies the output into
 * a package-owned artifact directory. By default this writes to nastech-cli/tools/webapp
 * for local development. Release packaging can pass --out-dir to place it elsewhere.
 *
 * nastech-cli does NOT depend on nastech-app — we reach into the sibling at build time only.
 */
...
const APP_DIR = path.resolve(REPO_ROOT, 'packages/nastech-app');
...
function main() {
    ...
    console.log(`→ Building nastech-app web bundle (expo export)`);
    rmrf(APP_DIST);
