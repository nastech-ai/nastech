...
import { nasTechClient, useNasTechState } from '@/nastech/client'
...
function HappyAuthPanel() {
    ...
    if (state.status === 'starting') {
        return <small className="plugin-detail__help">Loading NasTech state…</small>
    }
            ...
            <div className="plugin-detail__row">
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('logout', () => nasTechClient.logout())}
                        disabled={busy !== null}
                    >
                        Disconnect
                    </button>
    ...
    if (state.status === 'authenticating' && authUrl) {
        return (
            <div className="plugin-detail__row plugin-detail__row--column">
                <div className="nastech-auth__qr-wrap">
                    {qrDataUrl ? (
                        <img className="nastech-auth__qr" src={qrDataUrl} alt="NasTech authentication QR code" />
                    ) : (
                        <div className="nastech-auth__qr nastech-auth__qr--empty" />
                    )}
                </div>
                <div className="nastech-auth__url-row">
                    <input
                        className="plugin-detail__input nastech-auth__url"
                        value={authUrl}
                        readOnly
                        spellCheck={false}
                    />
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('cancel', () => nasTechClient.cancelAuth())}
                        disabled={busy !== null}
                    >
                        Cancel
                    </button>
        ...
        <div className="plugin-detail__row plugin-detail__row--column">
            ...
            <div className="plugin-detail__actions">
                <button
                    type="button"
                    className="plugins-page__action plugins-page__action--primary"
                    onClick={() => run('create', () => nasTechClient.createAccount())}
                    disabled={busy !== null}
                >
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('link', () => nasTechClient.startLinkDevice())}
                    disabled={busy !== null}
                >
            ...
            <div className="nastech-auth__url-row">
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('restore', () => nasTechClient.restoreSecret(secretKey))}
