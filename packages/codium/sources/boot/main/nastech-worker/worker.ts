...
if (!parentPort) {
    throw new Error('nastech worker must be started via worker_threads')
}
...
class HappyAuthenticatedClient {
    ...
    async request(path: string, init: RequestInit = {}): Promise<Response> {
        ...
        headers.set('X-NasTech-Client', this.cfg.clientId)
        return fetch(`${this.cfg.serverUrl}${path}`, { ...init, headers })
    }
}

port.on('message', (msg: NasTechWorkerRequestWithId) => {
    Promise.resolve()
        .then(() => handle(msg))
        .catch((err) => {
...
async function handle(msg: NasTechWorkerRequestWithId): Promise<void> {
...
async function startLinkDevice(requestId: string): Promise<void> {
    ...
    try {
        ...
        const flow: LinkFlow = {
            keypair,
            publicKey,
            authUrl: `nastech:///account?${publicKey}`,
            startedAt: Date.now(),
            cancelled: false,
        }
        ...
        try {
            ...
            if (data.state === 'authorized') {
                if (typeof data.token !== 'string' || typeof data.response !== 'string') {
                    throw new Error('NasTech auth response is missing token data')
                }
                const encrypted = decodeBase64(data.response)
                const secret = decryptBox(encrypted, flow.keypair.secretKey)
                if (!secret || secret.length !== 32) {
                    throw new Error('Failed to decrypt NasTech account credentials')
                }
                linkFlow = null
            ...
            if (!flow.cancelled && linkFlow === flow) {
                linkFlow = null
                setError(`NasTech link failed: ${errString(err)}`)
            }
            return
        }
        await delay(1000)
...
async function createEncryptionContext(masterSecret: Uint8Array): Promise<HappyEncryptionContext> {
    ...
    const anonId = encodeHex(
        (await deriveKey(masterSecret, 'NasTech', ['analytics', 'id'])).slice(0, 8),
    ).toLowerCase()
    const masterBlobKey = await deriveKey(masterSecret, 'NasTech Blobs', ['master'])
    return {
        anonId,
        contentPublicKey,
        contentSecretKey,
...
async function authGetToken(secret: Uint8Array): Promise<string> {
    ...
    if (typeof data.token !== 'string') {
        throw new Error('NasTech authentication did not return a token')
    }
    return data.token
}
...
async function apiJson(path: string, body: unknown): Promise<Record<string, unknown>> {
    const response = await fetch(`${config.serverUrl}${path}`, {
        ...
        headers: {
            ...
            'X-NasTech-Client': config.clientId,
        },
        body: JSON.stringify(body),
    })
    if (!response.ok) {
        const text = await response.text().catch(() => '')
        throw new Error(`NasTech API ${response.status}: ${text || response.statusText}`)
    }
...
function snapshotForCredentials(auth: StoredCredentials | null): NasTechStateSnapshot {
    if (!auth) {
        return {
            ...stateBase(),
            status: 'unconfigured',
...
function setState(next: NasTechStateSnapshot): void {
    state = next
...
function respond(requestId: string, next: NasTechStateSnapshot, value?: unknown): void {
