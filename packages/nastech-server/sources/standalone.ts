...
export async function runMigrations(opts: { pgliteDir: string; migrationsDir?: string } = { pgliteDir }) {
    ...
    candidates.push(
        ...
        path.join(process.cwd(), "packages", "nastech-server", "prisma", "migrations"),
...
async function serve() {
    ...
    if (process.env.NASTECH_INJECT_HTML_CONFIG) {
        try {
            injectHtmlConfig = JSON.parse(process.env.NASTECH_INJECT_HTML_CONFIG);
        } catch {
...
function findStaticDir(): string | undefined {
    const candidates = [
        process.env.NASTECH_STATIC_DIR,
...
const isDirectInvocation =
    ...
    invokedFile.endsWith("/nastech-server");
...
if (isDirectInvocation) {
    ...
    switch (command) {
        ...
        case "serve":
            ...
            break;
        default:
            console.log(`nastech-server - portable distribution
...
Environment variables:
  ...
  NASTECH_MASTER_SECRET  Required: master secret for auth/encryption
`);
