...
```bash
docker build -t nastech-server -f Dockerfile .
```
...
docker run -p 3005:3005 \
  -e NASTECH_MASTER_SECRET=<your-secret> \
  -v nastech-data:/data \
  nastech-server
```
...
Data persists in the `nastech-data` Docker volume across container restarts.
