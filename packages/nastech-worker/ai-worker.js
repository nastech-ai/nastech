...
export default {
  async fetch(request, env, ctx) {
    ...
    // Chat completions endpoint — support both paths
    if (
    ...
    if (url.pathname === '/v1/models') {
      return jsonResponse({
        ...
        data: [
          ...
          { id: '@cf/google/gemma-7b-it-lora', object: 'model', owned_by: 'cloudflare' },
        ],
      });
    }
...
async function handleChat(request, env) {
  ...
  // Route to custom provider if user has their own API key + base URL
