...
function createGitProject(envDir: string): { projectDir: string; worktreeDir: string } {
    const projectDir = join(envDir, 'nastech-agent-test-project');
    const worktreeDir = join(projectDir, '.dev', 'worktree', 'feature-branch');

    mkdirSync(projectDir, { recursive: true });
    runCommand('git', ['init', '--initial-branch=main'], projectDir);
    runCommand('git', ['config', 'user.name', 'NasTech Agent Test'], projectDir);
    runCommand('git', ['config', 'user.email', 'nastech-agent-tests@example.com'], projectDir);

    writeFile(join(projectDir, 'README.md'), '# NasTech Agent Test Project\n');
...
async function waitForSessionInList(sessionId: string, env: NodeJS.ProcessEnv): Promise<void> {
    ...
    }, 20_000, `session ${sessionId} to appear in nastech-agent list`);
}
...
async function approveAgentLogin(
    ...
    const response = await fetch(`${serverUrl}/v1/auth/account/response`, {
        ...
        headers: {
            ...
            'X-NasTech-Client': 'cli-control-plane/0.1.0',
        },
        body: JSON.stringify({
...
async function runAgentAuthLogin(env: NodeJS.ProcessEnv, approval: { serverUrl: string; token: string; secret: Uint8Array }): Promise<string> {
    return await new Promise<string>((resolvePromise, rejectPromise) => {
        ...
        const timeout = setTimeout(() => {
            ...
            settled = true;
            child.kill('SIGKILL');
            rejectPromise(new Error(`Timed out waiting for nastech-agent auth login.\n${stdout}\n${stderr}`));
        }, 60_000);
        ...
        child.on('close', code => {
            if (code !== 0) {
                finish(new Error(`nastech-agent auth login exited with code ${code}\n${stdout}\n${stderr}`));
                return;
            }
...
describe('nastech-agent integration', { timeout: 180_000 }, () => {
    beforeAll(async () => {
        ...
        if (keepIntegrationEnv) {
            console.log(`[nastech-agent integration] keeping environment: ${integrationEnvName}`);
            console.log(`[nastech-agent integration] environment dir: ${integrationEnvDir}`);
        }
    });
    ...
    it('spawns in the test project root and sends a message through nastech-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testProjectDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'nastech-agent root message';
    ...
    it('spawns in a git worktree and sends a message through nastech-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testWorktreeDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'nastech-agent worktree message';
        const spawnResult = parseJson<{
            type: 'success' | 'requestToApproveDirectoryCreation' | 'error';
            sessionId?: string;
            machineId?: string;
