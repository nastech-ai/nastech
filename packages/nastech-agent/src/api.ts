...
import type { SessionMessage as WireSessionMessage } from '@nastech-ai/nastech-wire';
...
function handleApiError(err: unknown, context: string): never {
    if (err instanceof AxiosError) {
        ...
        if (status === 401) {
            throw new Error('Authentication expired. Run `nastech-agent auth login` to re-authenticate.');
        }
        if (status === 403) {
