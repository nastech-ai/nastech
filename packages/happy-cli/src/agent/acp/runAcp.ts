...
import type { SessionEnvelope } from '@nastech-ai/nastech-wire';
...
export async function runAcp(opts: {
  ...
  if (response) {
    logAcp('muted', `NasTech Session ID: ${response.id}`);
  }
  ...
  const mcpServers = {
    nastech: {
      command: join(projectPath(), 'bin', 'nastech-mcp.mjs'),
  ...
  try {
    ...
    try {
      ...
      logger.debug(`[${opts.agentName}] Failed to stop NasTech MCP server:`, error);
    }
