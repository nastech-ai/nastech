...
export interface OfflineReconnectionConfig<TSession> {
    /** Server URL to health-check against (e.g., 'https://api.nastech.workers.dev') */
    serverUrl: string;
...
export function startOfflineReconnection<TSession>(
    ...
    const defaultHealthCheck = async () => {
        await axios.get(`${config.serverUrl}/v1/sessions`, {
            ...
            headers: {
                'X-NasTech-Client': `cli-daemon/${configuration.currentCliVersion}`
            }
        });
    };
    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `nastech auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `nastech auth`',
...
class OfflineState {
    ...
    private print(): void {
        const summary = [...this.failures.values()]
            ...
            .join('; ');
        console.log(`⚠️  NasTech server unreachable, offline mode with auto-reconnect enabled - error details: ${summary}`);
