 ...
 * Ensures consistent tmux handling across nastech-cli with proper session naming
 */
...
export class TmuxUtilities {
    ...
    public static readonly DEFAULT_SESSION_NAME = "nastech";
    ...
    async spawnInTmux(
        ...
        try {
            ...
            // - undefined: Use first existing session or create "nastech"
            // - empty string: Use first existing session or create "nastech"
            ...
            if (!sessionName) {
                ...
                if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
                    ...
                    // No sessions exist, create "nastech"
                    sessionName = 'nastech';
                    logger.debug(`[TMUX] No existing sessions, using default: ${sessionName}`);
                }
            }

            const windowName = options.windowName || `nastech-${Date.now()}`;
