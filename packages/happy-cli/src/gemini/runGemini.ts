 ...
 * through NasTech CLI. It manages the agent lifecycle, session state, and
 * communication with the NasTech server and mobile app.
 */
...
export async function runGemini(opts: {
  ...
  //
  // Fetch Gemini cloud token (from 'nastech connect gemini')
  //
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from NasTech cloud');
  ...
  // (assigned later after NasTech server setup)
  ...
  //
  // Start NasTech MCP server and create Gemini backend
  //

  const nasTechServer = await startNasTechServer(session);
  const bridgeCommand = join(projectPath(), 'bin', 'nastech-mcp.mjs');
  const mcpServers = {
    nastech: {
      command: bridgeCommand,
      args: ['--url', nasTechServer.url]
    }
  };
        ...
        if (msg.status === 'running') {
          ...
          if (errorMessage.includes('Authentication required')) {
            errorMessage = `Authentication required.\n` +
              `For Google Workspace accounts, run: nastech gemini project set <project-id>\n` +
              `Or use a different Google account: nastech connect gemini\n` +
          ...
          if (typeof error === 'object' && error !== null) {
            ...
            // Check for authentication error (Google Workspace accounts need project ID)
              ...
              errorMsg = `Authentication required. For Google Workspace accounts, you need to set a Google Cloud Project:\n` +
                         `  nastech gemini project set <your-project-id>\n` +
                         `Or use a different Google account: nastech connect gemini\n` +
                         `Guide: https://goo.gle/gemini-cli-auth-docs#workspace-gca`;
            }
