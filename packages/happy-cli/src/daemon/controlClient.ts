...
async function daemonPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.NASTECH_DAEMON_HTTP_TIMEOUT ? parseInt(process.env.NASTECH_DAEMON_HTTP_TIMEOUT) : 10_000;
...
export async function notifyDaemonSessionStarted(
  ...
  // app's resume-nastech-session RPC fails with "not tracked by this daemon".
 ...
 * For instance we are not handling the case where we upgraded nastech,
 ...
 * For instance when running `nastech daemon status` we can show more information.
 */
...
export async function isDaemonRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  // from the bundled version — e.g. when `nastech-coder@0.13.1` was published as
