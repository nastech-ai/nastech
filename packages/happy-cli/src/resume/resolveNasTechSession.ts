...
export type ResumableNasTechSession = {
    id: string;
    active: boolean;
    metadata: Metadata;
};
...
export function resolveSessionRecordByPrefix<T extends { id: string }>(records: T[], sessionId: string): T {
    ...
    if (matches.length === 0) {
        throw new Error(`No NasTech session found matching "${trimmed}"`);
    }
    if (matches.length > 1) {
        throw new Error(`Ambiguous NasTech session "${trimmed}" matches ${matches.length} sessions. Be more specific.`);
    }
    return matches[0];
}
...
function resolveSessionEncryption(session: RawSession, credentials: LocalHappyAgentCredentials): RecordEncryption {
    if (session.dataEncryptionKey) {
        ...
        if (!sessionKey) {
            throw new Error(`Failed to decrypt data key for NasTech session ${session.id}`);
        }
        return {
            key: sessionKey,
            variant: 'dataKey',
...
function decryptSessionMetadata(session: RawSession, credentials: LocalHappyAgentCredentials): Metadata {
    ...
    if (!metadata) {
        throw new Error(`Failed to decrypt metadata for NasTech session ${session.id}`);
    }

    try {
        return ResumableMetadataSchema.parse(metadata) as Metadata;
    } catch {
        throw new Error(`NasTech session ${session.id} is missing resumable metadata.`);
    }
}
    ...
    try {
        const response = await axios.get(`${configuration.serverUrl}/v1/sessions`, {
            headers: {
                ...
                'X-NasTech-Client': `cli-coding-session/${configuration.currentCliVersion}`,
            },
        });
        ...
        if (error instanceof AxiosError) {
            if (error.response?.status === 401) {
                throw new Error('NasTech session lookup authentication expired. Run `nastech-agent auth login` in this environment.');
            }
            throw new Error(`Failed to load NasTech sessions: ${error.message}`);
        }
        throw error;
    }
}

export async function resolveNasTechSession(sessionId: string): Promise<ResumableNasTechSession> {
