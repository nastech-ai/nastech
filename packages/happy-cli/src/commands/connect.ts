 ...
 * - connect codex: Store OpenAI API key in NasTech cloud
 * - connect claude: Store Anthropic API key in NasTech cloud
 * - connect gemini: Store Gemini API key in NasTech cloud
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('nastech connect')} - Connect AI vendor API keys to NasTech cloud

${chalk.bold('Usage:')}
  nastech connect codex        Store your Codex API key in NasTech cloud
  nastech connect claude       Store your Anthropic API key in NasTech cloud
  nastech connect gemini       Store your Gemini API key in NasTech cloud
  nastech connect status       Show connection status for all vendors
  nastech connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in NasTech cloud. This enables you to use these services through NasTech
...
${chalk.bold('Notes:')} 
  • You must be authenticated with NasTech first (run 'nastech auth login')
  • API keys are encrypted and stored securely in NasTech cloud
  • You can manage your stored keys at ba.nastech.workers.dev
`);
}

async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to NasTech cloud\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with NasTech'));
        console.log(chalk.gray('  Please run "nastech auth login" first'));
        process.exit(1);
    }
...
async function handleConnectStatus(): Promise<void> {
    ...
    console.log('');
    console.log(chalk.gray('To connect a vendor, run: nastech connect <vendor>'));
    console.log(chalk.gray('Example: nastech connect gemini'));
    console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with NasTech cloud
 * This ensures the Gemini SDK uses the same account as NasTech
 */
