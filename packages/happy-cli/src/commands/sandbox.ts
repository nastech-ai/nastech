...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `nastech sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('nastech sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  nastech sandbox configure      Configure sandbox settings interactively
  nastech sandbox status         Show current sandbox configuration
  nastech sandbox disable        Disable sandboxing
  nastech sandbox help           Show this help
`);
}
