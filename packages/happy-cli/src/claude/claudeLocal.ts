...
export async function claudeLocal(opts: {
    ...
    // We intercept these flags to use nastech-cli's session storage rather than Claude's default
    //
    // Supported patterns:
    ...
    try {
        ...
        // on macOS/Linux right after a remote→local switch (nastech-ai/nastech#301
        ...
        await new Promise<void>((r, reject) => {
            ...
            if (!claudeCliPath || !existsSync(claudeCliPath)) {
                throw new Error('Claude local launcher not found. Please ensure NASTECH_PROJECT_ROOT is set correctly for development.');
            }
