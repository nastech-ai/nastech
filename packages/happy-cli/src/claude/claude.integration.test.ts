 ...
 *   - NasTech MCP tool usage (`mcp__nastech__change_title`)
 ...
 *   - It does not exercise NasTech's separate local sandbox wrapper; instead it
...
import { startNasTechServer } from './utils/startNasTechServer';
...
class ClaudeQueryDriver {
    private nasTechServer: Awaited<ReturnType<typeof startNasTechServer>> | null = null;
    ...
    async start(): Promise<void> {
        ...
        this.nasTechServer = await startNasTechServer(fakeSessionClient);
    }

    stop(): void {
        this.nasTechServer?.stop();
        this.nasTechServer = null;
    }
...
describe.skipIf(!claudeAvailable)('Claude Integration (SDK/query)', { timeout: 180_000 }, () => {
    ...
    it('should clarify natively, resume across a model switch, use TodoWrite, and cross the project boundary with native tools', async () => {
        ...
        const execution = await driver!.runTurn({
            allowedTools: ['mcp__nastech__change_title', 'TodoWrite', 'TodoRead', 'Write', 'Edit', 'Read', 'Glob', 'LS'],
            ...
            prompt: [
                ...
                'Then update the nastech title so it mentions OPTION_B and reply with only DONE.',
            ].join('\n'),
        ...
        expect(execution.toolUseNames).toContain('mcp__nastech__change_title');
    ...
    it('should leave the file untouched and explain the refusal when native write is explicitly disallowed', async () => {
        const denied = await driver!.runTurn({
            allowedTools: ['mcp__nastech__change_title', 'Write', 'Edit', 'Read', 'LS'],
