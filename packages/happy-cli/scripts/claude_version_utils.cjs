 ...
 * Priority: NASTECH_CLAUDE_PATH env var > PATH > npm > Bun > Homebrew > Native
 * @returns {{path: string, source: string}|null} Path and source, or null if not found
 */
function findGlobalClaudeCliPath() {
    // 1. Environment variable (explicit override)
    const envPath = process.env.NASTECH_CLAUDE_PATH;
    if (envPath && fs.existsSync(envPath)) {
        const resolved = resolvePathSafe(envPath) || envPath;
        return { path: resolved, source: 'NASTECH_CLAUDE_PATH' };
    }
...
function runClaudeCli(cliPath) {
    ...
    //
    // CRITICAL: when nastech-cli aborts the launcher (terminal→remote switch),
    ...
    child.on('exit', (code, signal) => {
        // Mirror the binary's exit so nastech-cli sees the same status. If the
