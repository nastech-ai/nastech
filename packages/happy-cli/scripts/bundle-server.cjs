...
/**
 * Bundles nastech-server into a self-contained artifact directory.
 *
 * Uses `bun build --compile` to produce a single platform-specific binary, then copies the
 * pglite WASM/data files and prisma migrations alongside. nastech-cli does NOT depend on the
 * nastech-server workspace package — we reach into the sibling directory at build time only.
 *
 * Layout produced:
 *   <out-dir>/
 *     <platform>/
 *       nastech-server                  # bun-compiled binary
...
const SERVER_DIR = path.resolve(REPO_ROOT, 'packages/nastech-server');
...
function platformBinaryName(plat) {
    return plat.endsWith('-win32') ? 'nastech-server.exe' : 'nastech-server';
}
...
function buildPlatform(plat, outRoot) {
    ...
    console.log(`\n→ Bundling nastech-server for ${plat} (${target})`);
    run(
        'bun',
        [
            'build',
...
function main() {
    ...
    console.log(`\n✓ nastech-server bundle written to ${outDir}`);
}
