...
function printAvailableOptions() {
  console.error("Available nastech-app release options:");
...
async function promptForAction() {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    console.error("Interactive nastech-app release selection requires a TTY.");
  ...
  try {
    ...
    console.error(
      "Run `pnpm install` from repository root, then run `pnpm --filter nastech-app release` again."
    );
    process.exit(1);
  }

  const actionId = await select({
    message: "What should be released for nastech-app?",
    pageSize: 10,
...
async function main() {
  ...
  if (input && !action) {
    console.error(`Unknown nastech-app release option: ${input}`);
    printAvailableOptions();
    process.exit(1);
  }
  ...
  console.log(`Running nastech-app release option: ${action.label}`);
