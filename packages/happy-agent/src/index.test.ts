...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, NASTECH_HOME_DIR: '/tmp/nonexistent-nastech-test' } });
...
describe('nastech-agent CLI', () => {
    it('should display help output', () => {
        const { stdout } = runCli('--help');
        expect(stdout).toContain('nastech-agent');
        expect(stdout).toContain('CLI client for controlling NasTech agents remotely');
    });
