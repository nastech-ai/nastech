/**
 * CLI smoke tests for nastech-agent.
 *
...
const binPath = resolve(__dirname, '..', 'bin', 'nastech-agent.mjs');
...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, NASTECH_HOME_DIR: '/tmp/nonexistent-nastech-acceptance' } });
...
describe('Smoke: CLI command surface', () => {
    ...
    describe('2. list command', () => {
        ...
        it('fails with auth error when not authenticated', () => {
            ...
            expect(stderr).toContain('nastech-agent auth login');
        });
    });
...
describe('Smoke: Error handling', () => {
    ...
    describe('server error handling (via API error mapper)', () => {
        it('HTTP 401 maps to re-authenticate message', async () => {
            ...
            const errorMsg = 'Authentication expired. Run `nastech-agent auth login` to re-authenticate.';
            expect(errorMsg).toContain('nastech-agent auth login');
        });
...
describe('Smoke: Interop — dataKey vs legacy encryption', () => {
    ...
    it('session created with legacy encryption (nastech-cli style) can be decrypted', () => {
        const creds = makeCredentials();
    ...
    it('messages encrypted with dataKey can be round-tripped', () => {
        const sessionKey = getRandomBytes(32);
        const messageContent = { role: 'user', content: { type: 'text', text: 'Hello from nastech-agent' } };
...
describe('Smoke: Full test suite runs', () => {
    ...
    it('config loads with correct defaults', () => {
        const origUrl = process.env.NASTECH_SERVER_URL;
        const origHome = process.env.NASTECH_HOME_DIR;
        ...
        try {
            ...
            expect(config.homeDir).toContain('.nastech');
            expect(config.credentialPath).toContain('agent.key');
        } finally {
            if (origUrl !== undefined) process.env.NASTECH_SERVER_URL = origUrl;
            if (origHome !== undefined) process.env.NASTECH_HOME_DIR = origHome;
        }
    });
});
