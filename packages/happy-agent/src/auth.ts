...
export async function authLogin(config: Config): Promise<void> {
    ...
    try {
        await axios.post(`${config.serverUrl}/v1/auth/account/request`, {
            ...
            headers: { 'X-NasTech-Client': 'cli-control-plane/0.1.0' },
        });
    } catch (err) {
    ...
    const qrData = `nastech:///account?${encodeBase64Url(keypair.publicKey)}`;
    console.log('');
    qrcode.generate(qrData, { small: true }, (code: string) => {
        console.log(code);
    });
    console.log('## Authentication');
    console.log('- Action: Scan this QR code with the NasTech app');
...
export async function authStatus(config: Config): Promise<void> {
    ...
    if (creds) {
        ...
        console.log('- Action: Run `nastech-agent auth login` to authenticate.');
    }
}
