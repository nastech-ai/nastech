...
function getLatestStableTag() {
  const tagsRaw = execSync(`git tag --list "nastech-agent-v*" --sort=-v:refname`, {
    encoding: "utf8",
  });
  ...
  // Only accept stable semver tags like nastech-agent-v1.2.3 (no prerelease suffix).
  const stableTag = tags.find((t) => /^nastech-agent-v\d+\.\d+\.\d+$/.test(t));
...
async function generateReleaseNotes() {
  try {
    ...
    try {
      gitLog = execSync(
        `git log ${commitRange} --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/nastech-agent`,
        { encoding: "utf8" }
      );
    } catch (error) {
      console.error(
      ...
      gitLog = execSync(
        `git log -10 --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/nastech-agent`,
        { encoding: "utf8" }
      );
    }
    ...
    const prompt = `Please analyze these git commits and generate professional release notes for version ${toVersion} of the NasTech Agent CLI tool (a remote agent control CLI).

The release should cover commits since the latest stable nastech-agent tag (nastech-agent-vX.Y.Z): ${
      fromTag ?? "(none)"
    }.
