...
This is the short version of what matters most for NasTech.

| Vendor | Core transport | Transcript shape | Subagents / tasks | Permissions | Sandbox story | Sync / remote story | NasTech takeaway |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
- OpenCode is the most attractive end-to-end reference for NasTech right now.
...
## Suggested design direction for NasTech
...
- Evaluate Superset's Electric SQL cloud→local sync pattern and cloud DB command queue as alternatives to NasTech's current sync approach.
- Consider Superset's host-service extraction pattern (injectable providers, manifest-based durability) for NasTech's CLI/server split.
