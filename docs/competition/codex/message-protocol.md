...
This is a good reminder for NasTech: a list endpoint and a stream endpoint should not be the same thing.
...
This is a good template for representing delegated work inside NasTech without losing identity.
...
NasTech should copy this structure: normal event stream for state, explicit server requests for blocking decisions.

Primary source files:

- `../nastech-adjacent/research/codex/codex-rs/app-server-protocol/src/protocol/common.rs`
- `../nastech-adjacent/research/codex/codex-rs/app-server-protocol/src/protocol/v2.rs`
...
This is a strong reference for NasTech's server-side permission and sandbox contract.
...
NasTech should take this seriously if it wants robust mobile or multi-client session control.
