...
NasTech already has the runtime pieces needed for long-running and partially unsupervised agent work, but the guidance for how to operate, validate, and trust those workflows is scattered. This note captures:
...
- [`../../packages/nastech-cli/CLAUDE.md`](../../packages/nastech-cli/CLAUDE.md)
- [`../../packages/nastech-app/CLAUDE.md`](../../packages/nastech-app/CLAUDE.md)
- [`../../packages/nastech-server/CLAUDE.md`](../../packages/nastech-server/CLAUDE.md)
...
## How unsupervised development currently works in NasTech

Based on the current docs and codebase structure, the operating model is roughly:

1. The user starts `nastech`, `nastech codex`, `nastech gemini`, or another wrapped agent flow.
...
Important consequence: NasTech already has a protocol-level understanding of subagents and tool-spawned sidechains. The missing layer is not transport. The missing layer is process: when unsupervised operation is allowed, how it is validated, and what counts as evidence that it is safe and working.

## What is missing

There is no single document that answers these operator questions:

- What counts as an approved unsupervised workflow in NasTech?
...
That means NasTech has the runtime for autonomous behavior, but not the operating manual.
...
The most important ideas worth copying into NasTech are:
...
This matches NasTech's needs well. NasTech wraps multiple agents already, so a portable procedural layer is more valuable than yet another provider-specific prompt convention.
...
This lines up with NasTech's existing protocol support for `subagent` event streams. The runtime already understands subagent identity; the missing piece is guidance for when and how to use that capability well.

## Implications for NasTech

The practical conclusion is:

- NasTech does **not** need to invent subagent transport first.
- NasTech does **not** primarily need more architecture docs.
- NasTech **does** need an operator/process layer for autonomous work.
...
3. `docs/skills-and-subagents.md`
   ...
   - how NasTech should interoperate with existing agent skill directories

## Recommended design direction

If NasTech decides to adopt skills more explicitly, the most compatible path is likely:
...
- keep NasTech focused on orchestration, observability, and validation rather than becoming a separate skill runtime

That approach keeps NasTech aligned with the broader ecosystem while preserving its real value: remote control, daemonized execution, protocol normalization, and session observability across providers.

## Bottom line

NasTech currently has the substrate for unsupervised development, but not the handbook.
