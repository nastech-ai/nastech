...
[
  {
    "from": "ping",
    "text": "Here is the directory listing for /Users/.../nastech:\n\ncli\nDockerfile\n...",
...
1. **Force-enable agent teams** — set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` from NasTech so users get multi-agent capabilities without manual setup.
...
This is a lightweight UI feature with high visual impact — agent swarm activity showing up in the NasTech sidebar would trend on Twitter.
