...
How a single NasTech user is identified across every external service.

## Primary ID: NasTech Account CUID
...
```
NasTech Account CUID (e.g. cm4x7k2...)
│
...
POST /v1/auth { publicKey, challenge, signature }
  ...
  ├─ auth.createToken(CUID)  →  JWT (signed with NASTECH_MASTER_SECRET)
  │
  ▼
...
| System | ID Type | Why |
|--------|---------|-----|
| ElevenLabs | HMAC-derived | Privacy — raw NasTech ID never sent to ElevenLabs |
...
To derive an ElevenLabs user ID from a NasTech CUID locally:

```python
import hmac, hashlib, base64
digest = hmac.new(MASTER_SECRET.encode(), nastech_cuid.encode(), hashlib.sha256).digest()
