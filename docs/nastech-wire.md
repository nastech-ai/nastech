...
This document describes the shared wire package: `@nastech-ai/nastech-wire`.

## Why this package exists

Before `nastech-wire`, wire-level message and session-protocol schemas were duplicated across packages (CLI, app, server, and agent). That caused drift risk and made protocol evolution harder.

`@nastech-ai/nastech-wire` centralizes those shared schemas and types so all clients and services agree on the same wire contract.

## Package identity

- npm name: `@nastech-ai/nastech-wire`
- workspace path: `packages/nastech-wire`
...
Shared from `@nastech-ai/nastech-wire`:
...
### CLI (`packages/nastech-cli`)

- Session protocol imports now reference `@nastech-ai/nastech-wire` directly.
- `src/sessionProtocol/types.ts` now re-exports from `@nastech-ai/nastech-wire` as compatibility shim.
- API wire schemas in `src/api/types.ts` now source shared message/update schemas from `@nastech-ai/nastech-wire`.

### App (`packages/nastech-app`)

- Shared API message/update schemas in `sources/sync/apiTypes.ts` now import these from `@nastech-ai/nastech-wire`:
  - `ApiMessageSchema`
  - `ApiUpdateNewMessageSchema`
  - `ApiUpdateSessionStateSchema`
  - `ApiUpdateMachineStateSchema`

### Server (`packages/nastech-server`)

- Prisma JSON message content type now references `SessionMessageContent` from `@nastech-ai/nastech-wire`.
- Event router uses shared `SessionMessageContent` type for `new-message` payload typing.

### Agent (`packages/nastech-agent`)

- `RawMessage` now aliases `SessionMessage` from `@nastech-ai/nastech-wire`.

## Versioning model

All other workspace packages now declare a versioned dependency on `@nastech-ai/nastech-wire`.

This intentionally mirrors post-publish consumption and reduces hidden coupling to workspace-local files.

## Build and release

`@nastech-ai/nastech-wire` is configured the same way as existing publishable libraries in this repo:
...
```bash
yarn workspace @nastech-ai/nastech-wire release
```

When building workspaces from a clean checkout, build `@nastech-ai/nastech-wire` first so dependent packages can resolve generated `dist` outputs.
...
3. Bump and release `@nastech-ai/nastech-wire`.
4. Update downstream package versions if needed.
5. Publish dependent package updates only after the new `nastech-wire` version is available.

## Notes

- `nastech-wire` should stay focused on wire contracts only (types + Zod schemas + small helpers).
