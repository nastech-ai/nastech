...
1. Control plane: your NasTech session is visible to the manager in the shared
   NasTech space.
2. Code plane: your assigned git worktree.
3. Validation plane: your worktree-local NasTech env started with `yarn env:up`
   from that worktree.

Being visible in the shared NasTech space does not mean you should test in the
...
10. Report back in this NasTech session with exact commands and clear risks.
...
- Do not treat a toy nastech-path as sufficient validation when the feature is
...
- Exercise the feature under conditions closer to real NasTech usage: longer
...
- Report only through your own NasTech session.
