...
## Process lifecycle
Entry point: `packages/nastech-server/sources/main.ts`.
...
3. Initialize crypto modules:
   - `initEncrypt()` derives a KeyTree from `NASTECH_MASTER_SECRET`.
...
- Tokens are generated and verified by privacy-kit using `NASTECH_MASTER_SECRET`.
...
graph TB
    ...
    S1 & S2 & S3 & S4 --> |KeyTree from NASTECH_MASTER_SECRET| DB
...
- The server only encrypts/decrypts **service tokens** (GitHub OAuth tokens, vendor tokens) using the KeyTree derived from `NASTECH_MASTER_SECRET`.
...
- API server: `packages/nastech-server/sources/app/api/api.ts`
- Socket server: `packages/nastech-server/sources/app/api/socket.ts`
- Event routing: `packages/nastech-server/sources/app/events/eventRouter.ts`
- Presence: `packages/nastech-server/sources/app/presence`
- Storage: `packages/nastech-server/sources/storage`
- Prisma schema: `packages/nastech-server/prisma/schema.prisma`
