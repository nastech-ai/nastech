...
This document details how client data is encrypted, how encrypted blobs are structured, and how those blobs map onto protocol fields. It is based on `packages/nastech-cli/src/api/encryption.ts` and the server routes that accept/emit these values.
...
These are the client-side structures that get encrypted and sent over the wire. They are defined in `packages/nastech-cli/src/api/types.ts`.
...
These are encrypted with a server-only KeyTree derived from `NASTECH_MASTER_SECRET` and are not end-to-end encrypted.
...
- Client crypto: `packages/nastech-cli/src/api/encryption.ts`
- Session message format: `packages/nastech-cli/src/api/types.ts`
- Server message ingestion: `packages/nastech-server/sources/app/api/socket/sessionUpdateHandler.ts`
- Artifact/KV routes: `packages/nastech-server/sources/app/api/routes/artifactsRoutes.ts`, `packages/nastech-server/sources/app/kv/kvMutate.ts`
