# CLI V3 Messages API Migration (nastech-cli)

## Overview
Migrate `nastech-cli`'s `ApiSessionClient` from Socket.IO-based message read/write to the new v3 HTTP endpoints. The client will:
...
- **Package**: `packages/nastech-cli/src/api/apiSession.ts` — `ApiSessionClient` class (EventEmitter)
...
- **InvalidateSync**: Available in `src/utils/sync.ts` (identical to nastech-app's version)
...
- ⚠️ `npx eslint` in `packages/nastech-cli` currently fails because there is no `eslint.config.(js|mjs|cjs)` in this workspace, so lint verification could not be completed.
