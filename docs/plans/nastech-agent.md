...
- **Integration tests**: Use the real environment bootstrap (`yarn env:up:authenticated`) and exercise the live server + daemon + CLI stack. Do not use mocked acceptance coverage for `nastech-agent spawn`.
...
- [x] Create `packages/nastech-agent/` directory with `package.json` (name: `nastech-agent`, type: module, bin: `./bin/nastech-agent.mjs`)
- [x] Create `tsconfig.json` with strict mode, path aliases (`@/` → `src/`), ESM output
- [x] Create `bin/nastech-agent.mjs` entry point wrapper (mirrors nastech-cli pattern: spawns node with `--no-warnings`)
...
### Task 4: Authentication command (`nastech-agent auth`)
...
- [x] Add `nastech-agent auth login` subcommand that runs the flow above
- [x] Add `nastech-agent auth logout` subcommand that calls `clearCredentials()`
- [x] Add `nastech-agent auth status` subcommand that reads credentials and prints auth status (authenticated / not authenticated)
...
- [x] Create `src/index.ts` using `commander` with program name `nastech-agent`
- [x] `nastech-agent list` — calls `listSessions`, displays table: ID (truncated), name/summary, path, status (active/inactive), last active time. With `--json` outputs raw JSON. With `--active` filters to active only.
- [x] `nastech-agent status <session-id>` — fetches session via list + filter by ID prefix, connects Socket.IO to get live state, displays: session ID, metadata (path, host, lifecycle state), agent state (idle/busy, pending requests count), last message preview. With `--json` outputs raw JSON. Disconnects after displaying.
...
- [x] `nastech-agent create --tag <tag> [--path <path>]` — creates new session with given tag and metadata (path defaults to cwd, host to hostname). Prints session ID. With `--json` outputs full session JSON.
- [x] `nastech-agent send <session-id> <message>` — resolves session key, connects Socket.IO, sends user message (encrypted with AES-256-GCM), optionally waits for idle with `--wait`, and supports `--yolo` to send `permissionMode=yolo`. Disconnects after. Prints confirmation. With `--json` outputs message details.
...
- [x] `nastech-agent history <session-id>` — fetches messages via HTTP, resolves session encryption key (dataKey or legacy), decrypts each message, displays in chronological order with role/timestamp. With `--json` outputs raw JSON. With `--limit <n>` limits output.
- [x] `nastech-agent stop <session-id>` — connects Socket.IO, sends `session-end` event, disconnects. Prints confirmation.
- [x] `nastech-agent wait <session-id> [--timeout <seconds>]` — connects Socket.IO, waits for agent idle state (no pending requests, not controlled by user), prints when idle or times out (default 300s). Exit code 0 on idle, 1 on timeout.
...
- [x] Verify interop: session created by nastech-agent is visible and controllable from mobile app
- [x] Verify interop: session created by nastech-cli can be listed and history read by nastech-agent
- [x] Run full test suite (unit tests)
- [x] Run linter — all issues must be fixed

### Task 11: [Final] Update documentation
- [x] Add README.md to `packages/nastech-agent/` with usage examples for all commands
...
nastech-agent machines [--active] [--json]        # List machines
nastech-agent list [--active] [--json]            # List all sessions
nastech-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]  # Spawn on a machine
nastech-agent status <session-id> [--json]        # Get live session state
nastech-agent create --tag <tag> [--path <path>] [--json]  # Create new session
nastech-agent send <session-id> <message> [--yolo] [--wait] [--json]  # Send message
nastech-agent history <session-id> [--limit <n>] [--json]    # Read message history
nastech-agent stop <session-id>                   # Stop a session
nastech-agent wait <session-id> [--timeout <s>]   # Wait for agent to become idle
```
...
**For new sessions (created by nastech-agent):**
...
**For existing sessions (created by nastech-cli or other clients):**
...
- Test cross-client interop: messages from nastech-agent readable by mobile app and vice versa

**Distribution:**
- Package can be published to npm as `nastech-agent`
- Alternatively, users install from monorepo via `yarn workspace nastech-agent build`
