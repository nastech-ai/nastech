...
⚠️ No lint script is defined in `packages/nastech-server/package.json`, so linter execution is currently not available.
...
- Update `nastech-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `nastech-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
