...
- Server token route: `packages/nastech-server/sources/app/api/routes/voiceRoutes.ts`
- Client token fetch: `packages/nastech-app/sources/sync/apiVoice.ts`
- Voice start decision: `packages/nastech-app/sources/realtime/RealtimeSession.ts`
- ElevenLabs client session start:
  - `packages/nastech-app/sources/realtime/RealtimeVoiceSession.tsx`
  - `packages/nastech-app/sources/realtime/RealtimeVoiceSession.web.tsx`

## Existing Secret Assumptions

The repo already assumes ElevenLabs API access exists on the server:

- `packages/nastech-server/sources/app/api/routes/voiceRoutes.ts` reads `process.env.ELEVENLABS_API_KEY`.
- `packages/nastech-server/deploy/handy.yaml` extracts `/handy-elevenlabs`.
...
Update `packages/nastech-server/sources/app/api/routes/voiceRoutes.ts` to:
...
- `packages/nastech-app/sources/realtime/types.ts`
- `packages/nastech-app/sources/realtime/RealtimeVoiceSession.tsx`
- `packages/nastech-app/sources/realtime/RealtimeVoiceSession.web.tsx`
- `packages/nastech-app/sources/realtime/RealtimeSession.ts`
- `packages/nastech-app/sources/sync/apiVoice.ts`
...
There is older server-only free-trial work in the legacy `nastech-ai/nastech-server` repository on branch:
