...
Plan mode buttons (accept/reject) don't reliably appear in the NasTech app UI. Root cause: the CLI's `permissionHandler.handleToolCall` auto-approves ExitPlanMode when `permissionMode` is stale (`bypassPermissions` from a prior session), and `reset()` never clears `permissionMode`. Additionally, `isAborted` always returns true for ExitPlanMode as part of a PLAN_FAKE_RESTART hack, which causes a dead-end when the tool is auto-approved without injecting a restart message.
...
**File:** `packages/nastech-cli/src/claude/utils/permissionHandler.ts`
...
```bash
cd packages/nastech-cli && yarn add @anthropic-ai/claude-agent-sdk@^0.2.96
```
...
- Keep any NasTech-specific extensions as wrappers
...
Three tests against an isolated `/tmp/nastech-testing-ground-<random>/` fixture with `hello-world.js` in a git repo:
