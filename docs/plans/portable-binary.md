...
## Overview
Create a portable, self-contained distribution of nastech-server as a single Bun-compiled binary. It runs without Redis (already has in-memory event bus), uses PGlite for embedded PostgreSQL, and local filesystem for file storage. CLI provides `nastech-server migrate` and `nastech-server serve` commands.
...
- [ ] Add `@electric-sql/pglite`, `pglite-prisma-adapter` to nastech-server dependencies
...
- [ ] Add `build:standalone` script to `package.json`: `bun build ./sources/standalone.ts --compile --outfile nastech-server`
...
- [ ] Test: `./nastech-server migrate` creates and migrates a PGlite database
- [ ] Test: `./nastech-server serve` starts the server with PGlite

### Task 7: Verify end-to-end
- [ ] Build the binary
- [ ] Run `./nastech-server migrate` — verify database is created in `./data/`
- [ ] Run `./nastech-server serve` — verify server starts, health endpoint responds
...
**CLI usage:**
```
nastech-server migrate    # Apply database migrations
nastech-server serve      # Start the server
```

## Post-Completion
- Document env vars for portable mode (`PGLITE_DIR`, `DATA_DIR`, `NASTECH_MASTER_SECRET`)
- Test on Linux for cross-platform binary (Bun cross-compile)
- Consider adding `nastech-server init` command for first-time setup
