...
Add to `Settings` in `packages/nastech-cli/src/persistence.ts:37`:
...
If `settings.serverUrl` is set, everything uses it. If the server is dead, requests fail loudly. We do **not** auto-fall-back to `api.nastech.workers.dev`, do **not** auto-retry against another URL, do **not** silently switch modes.
...
`packages/nastech-cli/src/configuration.ts:32` becomes:
...
- **nastech-cli, nastech-agent, nastech-server: no analytics at all** (verified — see Audit reference below).
- **nastech-app (web + mobile): PostHog only**, already gated: `tracking = config.postHogKey ? new PostHog(...) : null` (`packages/nastech-app/sources/track/tracking.ts:4`). Every call site is `tracking?.capture(...)`, so absence of the key already short-circuits everything.
...
│   ├── webapp/                  pre-built nastech-app web export (~28 MB)
│   │   ├── index.html
│   │   ├── _expo/
│   │   └── assets/
│   ├── pglite/
...
**`packages/nastech-cli/scripts/build-server-assets.mjs`** (~50 LOC, build-time)
- `pnpm --filter @nastech-ai/nastech-app build:web`
- Copy webapp dist, pglite wasm, server migrations → `packages/nastech-cli/server-assets/`

**`packages/nastech-server/sources/index.ts`** (NEW, ~40 LOC)
...
| `@nastech-ai/nastech-server/index.ts` exports | ~40 |
| Build script | ~50 |
...
3. **Sharp dep in nastech-server** — ~20 MB native binary per platform. Either npm optional-deps (sharp does this itself) or stub out image processing for the self-host minimum.
4. **OAuth redirects in `connectRoutes.ts`** — hardcoded `https://ba.nastech.workers.dev`. For self-host either env-var (`PUBLIC_WEBAPP_URL`) or disable the GitHub-connect route entirely.
...
- The 5 hardcoded `ba.nastech.workers.dev` URLs in CLI / settings / system prompt.

## Audit reference (state of the repo when this plan was written)

### Analytics
Only `nastech-app` has PostHog (`packages/nastech-app/sources/track/tracking.ts:4`), already gated by `config.postHogKey`. CLI / agent / server have zero analytics (grepped).

### Server URL configurability (already exists)
- CLI / agent: `NASTECH_SERVER_URL` env var (default `https://api.nastech.workers.dev`).
- Web app: in-app Settings → Server screen + `EXPO_PUBLIC_NASTECH_SERVER_URL`.
- Server: `pnpm standalone:dev` runs with PGlite — no Docker, no Redis. Entry: `packages/nastech-server/sources/standalone.ts`.

### Hardcoded `ba.nastech.workers.dev` URLs (cosmetic)
- `nastech-app/sources/components/SettingsView.tsx:396` — privacy link
- `nastech-cli/src/ui/doctor.ts:225`, `src/claude/utils/systemPrompt.ts:20-23`, `src/commands/connect.ts:74` — docs / attribution
- `nastech-server/sources/app/api/routes/connectRoutes.ts` — GitHub OAuth redirect
