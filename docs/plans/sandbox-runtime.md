...
## Overview
Integrate `@anthropic-ai/sandbox-runtime` into nastech-cli to sandbox both **Claude Code** and **Codex** sessions with OS-level filesystem and network restrictions. The sandbox wraps agent subprocesses, enforcing configurable restrictions without requiring containers.

Key features:
- **`nastech sandbox configure`** - Interactive CLI wizard (using `inquirer`) to set up sandbox rules
- **`nastech sandbox status`** - Show current sandbox configuration
- **`nastech sandbox disable`** - Turn off sandboxing
- **Automatic enforcement** - Once configured, sandbox wraps both Claude and Codex sessions by default (bypass with `--no-sandbox`)
- **Global config** - Stored in `~/.nastech/settings.json` alongside existing settings
- **Dual agent support** - Same sandbox config applies to both Claude Code and Codex

## Context
- **Claude spawn point**: `packages/nastech-cli/src/claude/claudeLocal.ts:241` - `spawn('node', [claudeCliPath, ...args], {env, ...})`
- **Codex spawn point**: `packages/nastech-cli/src/codex/codexMcpClient.ts:107` - `StdioClientTransport({ command: 'codex', args: ['mcp-server'], env })` which internally calls `cross-spawn('codex', ['mcp-server'])`
- **Config storage**: `packages/nastech-cli/src/persistence.ts` - Settings interface + Zod schemas + atomic `updateSettings()`
- **Command dispatch**: `packages/nastech-cli/src/index.ts` - manual `if/else if` routing on `args[0]`
- **Existing command pattern**: `packages/nastech-cli/src/commands/connect.ts` - exported `handleXxxCommand(args)` functions
...
- [x] Run `yarn add @anthropic-ai/sandbox-runtime inquirer` in `packages/nastech-cli`
- [x] Run `yarn add -D @types/inquirer` in `packages/nastech-cli`
...
- [x] Create `packages/nastech-cli/src/sandbox/config.ts`
...
- [x] Create `packages/nastech-cli/src/sandbox/manager.ts`
...
### Task 5: Create `nastech sandbox configure` interactive wizard
- [x] Create `packages/nastech-cli/src/commands/sandbox.ts`
...
### Task 6: Implement `nastech sandbox status` and `nastech sandbox disable`
...
### Task 10: Add `nastech sandbox` to help text and polish
- [x] Add `nastech sandbox` to the help text in `index.ts` (alongside `auth`, `connect`, `daemon`, etc.)
...
- [x] Verify `nastech sandbox configure` walks through all questions and saves config (automated command tests)
- [x] Verify `nastech sandbox status` shows current config (automated command tests)
- [x] Verify `nastech sandbox disable` turns off sandbox (automated command tests)
...
- ⚠️ Lint blocker: `packages/nastech-cli` has no `eslint.config.*` / `.eslintrc*`, so ESLint 9 cannot run in this package.
...
**Manual verification:**
- Test `nastech sandbox configure` end-to-end on macOS
